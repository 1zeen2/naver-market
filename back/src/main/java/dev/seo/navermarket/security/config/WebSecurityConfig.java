package dev.seo.navermarket.security.config;
...
public class WebSecurityConfig implements WebMvcConfigurer {
    ...
    // JwtAuthenticationFilter는 @Component로 등록되어 있으므로, 직접 주입받아 사용합니다.
    // 별도의 @Bean 메서드로 생성할 필요가 없습니다.
    private final JwtAuthenticationFilter jwtAuthenticationFilter;
    private final JwtAuthenticationEntryPoint jwtAuthenticationEntryPoint;
    private final JwtAccessDeniedHandler jwtAccessDeniedHandler;

    // CustomUserDetailsService는 WebSecurityConfig에서 직접 사용되지 않으므로,
    // 필터 체인 구성에 필요한 의존성 주입 목록에서 제거합니다.
    // private final CustomUserDetailsService customUserDetailsService; // 제거됨
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .anonymous(AbstractHttpConfigurer::disable) // 익명 사용자 비활성화 (인증되지 않은 사용자도 접근 가능한 경로를 permitAll()로 명시적으로 처리)
                // 요청별 인가 규칙 설정:
                .authorizeHttpRequests(authorizeRequests -> authorizeRequests
                        .requestMatchers(HttpMethod.OPTIONS, "/**").permitAll() // OPTIONS 메서드는 CORS Pre-flight 요청을 위해 허용
                        ...
                        .requestMatchers("/uploads/**").permitAll() // 업로드된 정적 파일 접근 허용
                        // .requestMatchers("/h2-console/**").permitAll() // H2 Console 사용 시 주석 해제
                ...
                .addFilterBefore(jwtAuthenticationFilter, SecurityContextHolderFilter.class); // @Bean 메서드 호출 대신 직접 주입된 필터 사용
